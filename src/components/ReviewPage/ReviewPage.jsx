...
export default function ReviewPage({ cartItems = [] }) {
  ...
  return <div className="review-page" dir="rtl"><main className="review-container">
    ...
    <ol className="review-steps">
      <li className="done" onClick={() => navigate('/checkout')} style={{ cursor: 'pointer' }}><i>1</i><span>البيانات</span></li>
      <li className="done" onClick={() => navigate('/checkout/payment')} style={{ cursor: 'pointer' }}><i>2</i><span>الدفع</span></li>
      <li className="active" onClick={() => navigate('/checkout/review')} style={{ cursor: 'pointer' }}><i>3</i><span>المراجعة</span></li>
    </ol>
      ...
      <section className="review-details">
        ...
        <div className="review-info">
          <div>
            ...
            <button onClick={() => navigate('/checkout')}>تعديل</button>
          </div>
          ...
          <div>
            ...
            <section>
              <h2>طريقة الدفع</h2>
              <p>البطاقة البنكية (Visa •••• 3456)</p>
            </section>
            <button onClick={() => navigate('/checkout/payment')}>تعديل</button>
          </div>
          ...
          <div>
            ...
            <section>
              <h2>طريقة التوصيل</h2>
              <p>توصيل قياسي (2 - 5 أيام عمل)</p>
            </section>
            <button onClick={() => navigate('/checkout')}>تعديل</button>
          </div>
        </div>
      </section><aside className="review-summary"><h2>ملخص الطلب</h2><div><span>المجموع الفرعي</span><b>{subtotal} ر.س</b></div><div><span>الخصم (RASHAT10)</span><b className="discount">-{discount} ر.س</b></div><div><span>الشحن</span><b>{shipping} ر.س</b></div><hr/><div className="review-total"><span>الإجمالي</span><b>{total} ر.س</b></div><button onClick={handleConfirmOrder} disabled={loading}>{loading ? 'جاري التأكيد...' : 'تأكيد الطلب'} <img src="/icons/Container shopiing.svg" alt="" /></button></aside></div>
